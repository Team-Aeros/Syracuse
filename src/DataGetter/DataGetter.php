...
class DataGetter {
    ...
    public function __construct() {
        ...
        $currentDate = $this->dataReader->getCurrentDate('Europe/Amsterdam');
        $currentDateVals = explode("/",$currentDate);
        #var_dump($currentDateVals);
        $this->currentDay = mktime(0, 0, 0, $currentDateVals[1], trim($currentDateVals[2]), $currentDateVals[0]);
        ...
        if(!empty($this->gooddLinks)) {
            foreach ($this->goodLinks as $dataLink) {
                ...
                if ($file['date'] = $this->currentDate - 1) {
                    ...
                    } else if (in_array($linkparts[9], $this->dataReader->getGulfStations())) {
